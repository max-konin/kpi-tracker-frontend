...
describe('Acceptence | Tasks | Task | Edit Page', function() {
  ...
  describe('visit /tasks/:id/edit', function () {
    ...
    context("when current user is not task's owner", function () {
      beforeEach(async function () {
        server.createList('category', 2);
        const task = server.create('task');
        await authenticateSession({ email: 'me@exmple.com' });
        await visit(`tasks/${task.id}/edit`);
      });
      it('redirects to /tasks', function () {
        expect(currentURL()).to.eq('/tasks');
      });
    });
    context("when current user is task's owner", function () {
      beforeEach(async function () {
        server.createList('category', 2);
        const task = server.create('task', { userId: 'me' });
        await authenticateSession({ email: 'me@exmple.com' });
        await visit(`tasks/${task.id}/edit`);
      });
